...
namespace sharpPDF
{
	...
	public class pdfPage : IWritable
	{
		...
		public void newAddImageV2 ( Texture2D texture,  int X, int Y ) {
			Byte[] byteT = texture.EncodeToJPG ();
			addImage(byteT,X,Y,texture.height/2,texture.width/2);
			Debug.Log ( "Adding Image Length "+byteT.Length+" HEIGTH :"+texture.height+" WIDTH :"+texture.width );
		}
        public void newAddImageV3(Texture2D texture, int X, int Y)
        {
            Byte[] byteT = texture.EncodeToJPG();
			addImage (byteT, X, Y, texture.height, texture.width);
			         Debug.Log("Adding Image");
        }
		public void newAddImageV4(Texture2D texture, int X, int Y)
		{
			Byte[] byteT = texture.EncodeToJPG();
			addImage (byteT, X, Y, texture.height, texture.width);
			Debug.Log("Adding Image");
